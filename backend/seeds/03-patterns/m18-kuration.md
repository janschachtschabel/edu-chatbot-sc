...
tools:
  ...
  # ── Lesend, aber hier unentbehrlich ───────────────────────────────
  # Kuratieren heisst fast immer: erst den Gegenstand finden und seinen
  # IST-Stand lesen, dann ändern. Ohne sie müsste das Modell mit geratenen IDs
  # schreiben — genau der Fehler, den die Vorschau abfängt, aber erst NACH
  # einem überflüssigen Zug.
  # H5: unter WELCHEM Namen geschrieben würde. Vor dem Ankündigen einer
  # Änderung die ehrlichere Auskunft als eine Vermutung — und bei
  # authenticated=false erklärt es, warum gleich ALLES fehlschlägt.
  - wlo_auth_status
  - get_node_details
...
# HÖCHSTENS FÜNF Einträge je Liste — der Klassifikator-Prompt schneidet danach
# ab (``classify_prompt_blocks._render_patterns_hint_block``, ALT-verbatim).
# Eine sechste Zeile sieht im Studio gepflegt aus und erreicht das Modell nie;
# gepinnt von ``test_jede_gepflegte_zeile_erreicht_den_klassifikator``. Beim
# Ergänzen also zusammenfassen statt anhängen — die Verben stehen als Beispiele
# in Klammern, das kostet keine Zeile.
when_to_use:
...
# Fünf möglichst VERSCHIEDENE Verben schlagen acht ähnliche: „Erstell eine
# Sammlung dafür" trug dasselbe wie „Leg das in WLO an", während „Lösch den
# Eintrag" — der einzige zerstörende Fall — hinten abgeschnitten wurde.
trigger_phrases:
  - Leg das in WLO an
