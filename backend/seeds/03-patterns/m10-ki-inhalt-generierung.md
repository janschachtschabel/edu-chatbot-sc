...
sources:
  - llm
tools:
  # D2: „Arbeitsblatt / Quiz / Prüfung erstellen" ist der zweite Fall, für den
  # die Redaktion Skills schreibt.
  - search_skill
  - get_skill
  ...
  - get_skill_registry
  # H5: eine unabhängige Quelle, um eine selbst erzeugte fachliche
  # Aussage gegenzuhalten, bevor sie als Material herausgeht. Anriss zum
  # Klären; für den vollen Artikel get_url_text mit der gelieferten
  # Adresse. KEIN Ersatz für den WLO-Bestand.
  - get_wikipedia_summary
  - get_url_text
  # KEINE Suchwerkzeuge, und das ist Absicht. Die Skill-Anbindung trug hier
  # kurzzeitig search_wlo_collections + search_wlo_topic_pages ein, mit der
  # Begründung, M10 habe sie zuvor über den Rückfall-Zweig von
  # _select_active_tools bekommen. Das stimmt nicht: der Zweig ist unerreichbar
  # (test_rueckfall_zweig_ist_unerreichbar), M10 bekam im Betrieb NULL
  # Werkzeuge. Sie einzutragen bewahrte also nichts, sondern vergab neu — gegen
  # die eigenen forbidden_phrases („Such-Tool-Calls", „Hier sind passende
  # Sammlungen") und gegen die core_rule „niemals Such-CTA". Dazu hängt
  # phase3_modulate an jedes Suchwerkzeug noch zwei Helfer.
  # Gepinnt von TestKiErzeugungSuchtNicht (R1, 2026-08-11).
precondition_slots:
  - thema
core_rule: |
...
if material_type ∈ {bericht, factsheet, kennzahlen, steckbrief, vergleich, pressemitteilung}:
    ...
    # Lern-Material: KEIN Tool-Call, direkt aus LLM-Wissen
```

### Schritt 2 — Markdown-Antwort rendern
