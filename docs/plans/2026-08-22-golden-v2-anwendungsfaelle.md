...
| Prod-Befund 2: Gold-Runner fehlt im Image | ✅ 2026-08-22 — nach dem EVAL_CHAT_URL-Fix starb der erste Prod-Gold-Lauf sofort: „[Errno 2] No such file or directory: '/app/.venv/lib/evals/run_golden.py'" (NUR Gold — der generative Runner ist ein Paket-Modul in der venv und lief). Doppelte Wurzel: (a) `golden_run.py` löste den Runner rein über Quellbaum-Arithmetik auf (`parents[5]/evals/…` — im installierten Paket zeigt das auf `/app/.venv/lib/…`), (b) das Dockerfile kopierte `evals/` nie ins Image. Fix: `_runner_path()` mit Umgebungs-Vorrang `EVAL_RUNNER_PATH` (sonst Quellbaum-Heuristik) + klare Fehlermeldung mit Stellschraube statt nacktem Errno 2; Dockerfile kopiert `evals/run_golden.py` nach `/app/evals/` und setzt `ENV EVAL_RUNNER_PATH=/app/evals/run_golden.py`. 4 neue Wächter (3× Resolver in `test_eval_router.py`, 1× Dockerfile-Inhalt in `test_deploy_compose.py`; alle rot vor dem Fix). **Wirksam erst mit neu gebautem Image**; Übergangs-Trick auf dem Server ohne Rebuild: `evals/run_golden.py` als Bind-Mount auf exakt `/app/.venv/lib/evals/run_golden.py` legen (der Pfad, den das ALTE Image auflöst). **Nachbefund aus dem ersten Image-Build:** der neue COPY scheiterte mit „not found" — die `.dockerignore` schloss `evals` PAUSCHAL aus dem Build-Kontext aus (ihr Kommentar „Nothing the running server reads" war seit P10-1 faktisch falsch: der Backend lädt die Runner-Datei zur Laufzeit; im Image nur nie ausgeübt). Fix: Pauschal-Zeile raus (reports/ bleibt ausgeschlossen, __pycache__ global), Kommentar richtiggestellt, und der Dockerfile-Wächter prüft jetzt AUCH die `.dockerignore` auf evals-Pauschal-Muster (rot vor dem Fix). |
