...
describe("LoginComponent", () => {
  ...
  it("labels the password field properly", async () => {
    const h = await mount();
    const label = h.el.querySelector<HTMLLabelElement>(
      'label[for="studio-password"]',
    );
  ...
  it("explains itself when the session expired mid-work", async () => {
    ...
    expect(h.el.querySelector("#pw-hint")?.textContent).toContain(
  ...
  it("lässt sich umschalten, BEVOR man sich anmeldet", async () => {
    ...
    await h.fixture.whenStable();
    expect(
      h.el.querySelector('label[for="studio-password"]')?.textContent?.trim(),
    ).toBe("Password");
    expect(h.el.querySelector("#pw-hint")?.textContent?.trim()).toBe(
  ...
  it("übersetzt auch die Fehlermeldungen — sie sind Oberfläche, kein Protokoll", async () => {
    ...
    await h.fixture.whenStable();
    await submit(h, 401);
    expect(h.el.querySelector("#pw-error")?.textContent).toContain(
      "Wrong password",
    );
  });
