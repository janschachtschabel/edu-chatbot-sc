  ...
  patterns: [
    {
      id: "M01",
      label: "Orientierung",
      file: "03-patterns/m01-orientierung.md",
    },
    { id: "M02", label: "Suche", file: "03-patterns/m02-suche.md" },
  ],
  personas: [
    { id: "lehrkraft", label: "Lehrkraft", file: "04-personas/lehrkraft.md" },
  ],
};
...
function answerDoc(
  ...
  http
    .expectOne(`/studio/api/config/schema/${key}`)
    .flush(AREA_SCHEMAS["03-patterns"]);
  http
    .expectOne(`/studio/api/config/data/${key}`)
    .flush({ area: key, data, type: "md" });
}

describe("GroupSectionComponent", () => {
  it("lists the documents of its group with their labels", async () => {
    const { el } = await mount();
    const options = Array.from(el.querySelectorAll(".gs-entry")).map((b) =>
      b.textContent?.trim(),
    );
  ...
  it("loads a document when it is picked, addressed by its own key", async () => {
    ...
    answerDoc(http, "03-patterns/m02-suche", {
      frontmatter: { id: "M02" },
      body: "# Suche",
    });
    await tick();
  ...
  it("saves the WHOLE document of the selected entry", async () => {
    ...
    const req = http.expectOne(
      "/studio/api/config/data/03-patterns/m01-orientierung",
    );
  ...
  it("clears a refusal notice once something actually happens", async () => {
    ...
    await fixture.whenStable();
    http
      .expectOne("/studio/api/config/data/03-patterns/m01-orientierung")
      .flush({
    ...
    expect(el.querySelector(".cs-status")?.textContent?.trim()).toBe(
      "Gespeichert.",
    );
  });
...
async function mountWithDoc(section: CuratedSection): Promise<Harness> {
  ...
  answerDoc(h.http, "03-patterns/m01-orientierung", {
    frontmatter: { id: "M01", label: "Orientierung", tools: [] },
    body: "# Text",
  });
  await tick();
  await h.fixture.whenStable();
