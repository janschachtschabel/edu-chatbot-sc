...
describe("RagDocumentsComponent", () => {
  ...
  it("tells the page a document is gone, so the area counts stop lying", async () => {
    ...
    await fixture.whenStable();
    http
      .expectOne((r) => r.url === "/studio/api/rag/area/wlo/doc")
      .flush({ status: "deleted" });
    await tick();
    await fixture.whenStable();
  ...
  it("loads the full text on request — the preview is 200 characters", async () => {
    ...
    req.flush({
      ...
      total_chars: 40,
      chunks: [
        { index: 0, content: "Abschnitt eins", created_at: "" },
    ...
    const chunks = Array.from(el.querySelectorAll(".rd-chunk-body")).map(
      (c) => c.textContent,
    );
