...
const SESSIONS = [
  {
    session_id: 'abc-123', persona_id: 'lehrkraft', state_id: 'S3', turn_count: 7,
    created_at: '2026-07-20T10:00:00Z', updated_at: '2026-07-24T18:30:00Z',
  },
  {
    session_id: 'def-456', persona_id: '', state_id: 'S1', turn_count: 1,
    created_at: '2026-07-24T09:00:00Z', updated_at: '2026-07-24T09:01:00Z',
  },
];
...
async function mount(sessions: unknown[] = SESSIONS, locale = 'de'): Promise<Harness> {
...
describe('SessionsComponent', () => {
  ...
  it('names a session without a persona instead of showing a gap', async () => {
    const { el } = await mount();
    expect(el.querySelectorAll('.sv-row')[1].textContent).toContain('unbekannt');
  });
  ...
  it('reports a failed list instead of showing zero sessions', async () => {
    ...
    const http = TestBed.inject(HttpTestingController);
    await fixture.whenStable();
    http.expectOne('/studio/api/sessions/').flush('x', { status: 503, statusText: 'Unavailable' });
    await tick();
    await fixture.whenStable();
  ...
  it('loads the transcript of the chosen session, capped at the endpoint limit', async () => {
    const h = await mount();
    await select(h, 0);
    const req = h.http.expectOne((r) => r.url === '/studio/api/sessions/abc-123/messages');
  ...
  it('stays on the session whose history was cleared, and re-reads it', async () => {
    ...
    const h = await mount();
    await select(h, 0);
    h.http.expectOne((r) => r.url === '/studio/api/sessions/abc-123/messages')
    ...
    h.el.querySelector<HTMLButtonElement>('.sv-clear-yes')?.click();
    await h.fixture.whenStable();
    h.http.expectOne('/studio/api/sessions/abc-123/messages').flush({ status: 'cleared' });
    await tick();
    await h.fixture.whenStable();
    ...
    h.http.expectOne((r) => r.url === '/studio/api/sessions/abc-123/messages').flush([]);
    await tick();
    await h.fixture.whenStable();
 ...
 */
  ...
  it('gibt beiden Knöpfen EINEN Namen, der mit dem sichtbaren Wort beginnt', async () => {
    ...
    expect(clear.getAttribute('aria-label')).toBe('Verlauf leeren — Session abc-123');
