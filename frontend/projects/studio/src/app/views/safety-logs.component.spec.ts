...
async function mount(
  ...
  await h.fixture.whenStable();
  h.http
    .expectOne((r) => r.url === LOGS_URL)
    .flush({ count: logs.length, logs });
...
describe("SafetyLogsComponent", () => {
  ...
  it("keeps the list usable when only the stats fail", async () => {
    ...
    expect(
      h.el.querySelector('.sfl-stats-state [role="alert"]'),
    ).not.toBeNull();
  });
  ...
  it("drops the detail of an event the new filter no longer lists", async () => {
    ...
    const h = await mount();
    await pick(h, 1);
    expect(h.el.querySelector(".sfl-detail")!.textContent).toContain(
      "Musterstraße",
    );
