...
@Component({
  ...
  template:
    '<studio-quality-matrix [scope]="scope()" (drill)="last = $event" />',
})
class HostComponent {
...
describe("QualityMatrixComponent", () => {
  ...
  it("gives the grid row and column headers", async () => {
    const h = await mount();
    const cols = Array.from(h.el.querySelectorAll('th[scope="col"]')).map(
      (t) => t.textContent,
    );
    const rows = Array.from(h.el.querySelectorAll('th[scope="row"]')).map(
      (t) => t.textContent,
    );
  ...
  it("reports a failed read and keeps the grid it already had", async () => {
    ...
    h.http
      ...
      .flush(
        { detail: "Matrix-Query abgebrochen." },
  ...
  it("setzt Turn-Zahl und Sample-Zahl in die Mehrzahl der Sprache", async () => {
    ...
    expect(h.el.querySelector(".qm-total")!.textContent).toContain(
      "aus 1 Turn (all)",
    );
  ...
  it("nimmt Beschriftungen und den Vorlese-Text des Drilldowns aus dem Katalog", async () => {
    ...
    expect(h.el.querySelector(".qm-field")!.textContent).toContain(
      "Min samples per cell",
    );
    expect(cellButtons(h)[0].querySelector(".sr")!.textContent).toBe(
      "Show logs for I02",
    );
  });
});
