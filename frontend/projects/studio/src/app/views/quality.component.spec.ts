...
const STATS = {
  scope: 'all', total_turns: 120, pattern_distribution: { M04: 60 },
  intent_distribution: { I02: 70 }, avg_confidence: 0.8, degradation_rate: 0.01,
  empty_entity_rate: 0.1, avg_response_length: 400,
};

const BREAKDOWN = { scope: 'all', total: 0, groups: [], turns: [], max_confidence: 0.6 };
...
async function flushOverview(h: Harness): Promise<void> {
  ...
  for (const path of ['degradations', 'empty-entities', 'low-confidence']) {
    h.http.expectOne((r) => r.url === `/studio/api/quality/${path}`).flush(BREAKDOWN);
  }
}
...
async function openTab(h: Harness, label: string): Promise<void> {
  tabs(h).find((t) => t.textContent!.includes(label))!.click();
  await h.fixture.whenStable();
}
...
describe('QualityComponent', () => {
  it('offers four tabs, each pointing at a panel that exists', async () => {
    ...
    expect(labels).toEqual(['Übersicht', 'Routing-Matrix', 'Gesprächs-Flow', 'Logs']);
  ...
  it('opens on the overview and hides the other three panels', async () => {
    ...
    expect(h.el.querySelector('#panel-uebersicht')!.hasAttribute('hidden')).toBe(false);
    expect(h.el.querySelector('#panel-matrix')!.hasAttribute('hidden')).toBe(true);
  });
  ...
  it('switches the scope with real radios, and every open panel follows', async () => {
    ...
    h.http.expectOne((r) => r.url === FLOW_URL).flush({
      scope: 'all', days: 30, total_turns: 0, total_transitions: 0,
      state_distribution: {}, transitions: [],
    });
    await settle(h);

    const radios = Array.from(h.el.querySelectorAll<HTMLInputElement>('input[type="radio"]'));
    ...
    expect(h.http.expectOne((r) => r.url === FLOW_URL).request.params.get('scope')).toBe('eval');
    await flushOverview(h);
    await settle(h);
  });
  ...
  it('sends a drill-down to the log tab and applies it as a filter', async () => {
    const h = await mount();
    await openTab(h, 'Routing-Matrix');
    h.http.expectOne((r) => r.url === MATRIX_URL).flush({
      scope: 'all', total_turns: 10, cells: [{
        persona_id: 'P01', intent_id: 'I05', top_pattern: 'M15',
        top_pattern_count: 4, total_count: 5, share: 0.8, alternatives: [],
      }],
    });
    await settle(h);
    ...
    const selected = tabs(h).find((t) => t.getAttribute('aria-selected') === 'true');
