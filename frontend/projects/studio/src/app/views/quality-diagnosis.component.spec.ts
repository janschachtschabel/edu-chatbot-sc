...
const DEGRADATIONS = {
  scope: 'all', total: 12,
  groups: [{
    pattern_id: 'M10', missing_slots: ['thema', 'material_typ'], count: 7,
    example_message: 'Mach mir was', example_intent: 'I04', example_persona: 'P02',
  }],
};

const EMPTY_ENTITIES = {
  scope: 'all', total: 5,
  groups: [{ intent_id: 'I02', pattern_id: 'M04', count: 5, example_message: 'Hä?' }],
};

const LOW_CONFIDENCE = {
  scope: 'all', total: 2, max_confidence: 0.6,
  turns: [{
    id: 9, message: 'irgendwas', intent_id: 'I07', pattern_id: 'M15', persona_id: 'P01',
    state_id: 'S2', final_confidence: 0.31, created_at: '2026-07-24T10:00:00Z',
  }],
};
...
@Component({
  ...
  template: '<studio-quality-diagnosis [scope]="scope()" (drill)="last = $event" />',
})
class HostComponent {
...
async function mount(options: {
  ...
  http.expectOne((r) => r.url === DEG_URL).flush(options.degradations ?? DEGRADATIONS);
  const entities = http.expectOne((r) => r.url === ENT_URL);
  if (options.failEntities) entities.flush({ detail: 'Kaputt.' }, { status: 500, statusText: 'x' });
  else entities.flush(options.entities ?? EMPTY_ENTITIES);
  http.expectOne((r) => r.url === CONF_URL).flush(options.confidence ?? LOW_CONFIDENCE);
...
describe('QualityDiagnosisComponent', () => {
  ...
  it('shows a failing breakdown as failed and leaves the other two alone', async () => {
    ...
    expect(boxes[1].querySelector('[role="alert"]')!.textContent).toContain('Kaputt.');
  ...
  it('distinguishes an empty breakdown from a broken one', async () => {
    const h = await mount({ degradations: { scope: 'all', total: 0, groups: [] } });
    const box = h.el.querySelectorAll('details')[0];
  ...
  it('re-reads all three when the scope changes', async () => {
    ...
    for (const url of [DEG_URL, ENT_URL, CONF_URL]) {
      ...
      req.flush({ scope: 'production', total: 0, groups: [], turns: [], max_confidence: 0.6 });
    }
    await settle(h);
  });
});
