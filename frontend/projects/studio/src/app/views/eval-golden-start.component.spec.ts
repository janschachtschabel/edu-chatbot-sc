...
const FLOWS = {
  flows: [
    {
      id: "GS-1",
      title: "Lehrkraft: Material → Lernpfad",
      persona: "P-LEH",
      intents: ["I03", "I04"],
      turns: [{}, {}, {}, {}],
    },
...
async function mount(
  payload: Record<string, unknown> | null = FLOWS,
  locale = "de",
): Promise<void> {
...
describe("EvalGoldenStartComponent", () => {
  ...
  it("surfaces the flow ids the backend dropped", async () => {
    ...
    await h.fixture.whenStable();
    h.http
      .expectOne((r) => r.url === START_URL)
      .flush({
        run_id: "eval-g1",
        warnings: ["Unbekannte Flow-IDs ignoriert: ['GS-99']"],
      });
    await settle();
  ...
  it("setzt beide Anzahlen der Kostenzeile in die Mehrzahl der Sprache", async () => {
    await mount({
      flows: [{ id: "GS-9", title: "Einzelfall", turns: [{}] }],
      count: 1,
    });
    const cost = h.el
      .querySelector(".egd-cost")!
  ...
  it("trägt die Mehrzahl auch in die Rückfrage", async () => {
    ...
    expect(
      h.el
        .querySelector('.egd-confirm [role="alert"]')!
        .textContent!.replace(/\s+/g, " ")
        .trim(),
    ).toBe(
      "7 Chat-Anfragen plus 7 Judge-Aufrufe starten? Es läuft dann kein " +
