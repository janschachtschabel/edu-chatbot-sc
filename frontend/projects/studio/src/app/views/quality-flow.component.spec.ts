...
const captions = (h: Harness): (string | null)[] =>
  Array.from(h.el.querySelectorAll('caption')).map((c) => c.textContent!.trim());
...
describe('QualityFlowComponent', () => {
  ...
  it('reports a failed read', async () => {
    ...
    const h = { fixture, el: fixture.nativeElement as HTMLElement, http };
    await fixture.whenStable();
    http.expectOne((r) => r.url === FLOW_URL)
      .flush({ detail: 'Zeitfenster zu groß.' }, { status: 400, statusText: 'x' });
    await settle(h);
