...
async function mount(
  response: object = [
    { path: '01-base/welcome-config.yaml', full_path: 'x', name: 'welcome-config.yaml', type: 'yaml' },
    { path: '01-base/policy.yaml', full_path: 'x', name: 'policy.yaml', type: 'yaml' },
    { path: '03-patterns/m01-krisen.md', full_path: 'x', name: 'm01-krisen.md', type: 'md' },
  ],
  status = 200,
): Promise<Harness> {
...
describe('AreasComponent', () => {
  ...
  it('groups by folder, folders and entries sorted', async () => {
    const h = await mount();
    const folders = Array.from(h.el.querySelectorAll('.ar-folder')).map((n) => n.textContent);
    expect(folders).toEqual(['01-base', '03-patterns']);
    const first = Array.from(h.el.querySelectorAll('.ar-group')[0].querySelectorAll('.ar-name'));
    expect(first.map((n) => n.textContent)).toEqual(['policy', 'welcome-config']);
  });
