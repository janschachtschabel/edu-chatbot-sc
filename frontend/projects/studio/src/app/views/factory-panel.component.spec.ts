...
describe('FactoryPanelComponent', () => {
  ...
  it('zeigt Datum und Bezeichnung des gesicherten Standes', async () => {
    const h = await mount({
      exists: true, created_at: '2026-07-20T09:30:00Z', label: 'Auslieferung',
    });
  ...
  it('fragt vor dem Überschreiben des Werksstands zurück', async () => {
    const h = await mount({ exists: true, created_at: '2026-07-20T09:30:00Z', label: 'alt' });
    ...
    h.http.expectOne(`${FACTORY}/save`).flush({ status: 'saved', id: 'factory' });
    await settle(h);
    // The card must show the NEW date, so the status is re-read.
    h.http.expectOne(FACTORY).flush({
      exists: true, created_at: '2026-07-26T12:00:00Z', label: 'factory',
    });
    await settle(h);
  ...
  it('nennt beim Zurücksetzen die echte Folge und meldet die Zahl der Bereiche', async () => {
    const h = await mount({ exists: true, created_at: '2026-07-20T09:30:00Z', label: 'x' });
    ...
    h.http.expectOne(`${FACTORY}/restore`).flush({ status: 'restored', areas: 35 });
    await settle(h);
  ...
  it('lädt einen Werksstand erst nach Rückfrage hoch', async () => {
    ...
    await settle(h);
    h.http.expectOne(FACTORY).flush({ exists: true, created_at: '2026-07-26T12:00:00Z', label: 'f' });
  });
});
