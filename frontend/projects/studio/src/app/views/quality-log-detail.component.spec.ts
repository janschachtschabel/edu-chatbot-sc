...
@Component({
  ...
  template:
    '<studio-quality-log-detail [log]="log()" (dismiss)="closed = true" />',
})
class HostComponent {
...
describe("QualityLogDetailComponent", () => {
  ...
  it("drops entities whose value is empty instead of printing a blank row", async () => {
    const h = await mount();
    const keys = Array.from(h.el.querySelectorAll(".qd-entities th")).map(
      (t) => t.textContent,
    );
  ...
  it("says a clean turn was clean rather than showing nothing", async () => {
    const h = await mount();
    expect(h.el.querySelector(".qd-degradation")!.textContent).toContain(
      "Keine Degradation",
    );
  });
  ...
  it("nimmt Feldnamen und den Degradations-Satz aus dem Katalog", async () => {
    const h = await mount(
      { ...TURN, degradation: 1, missing_slots: ["thema"] },
      "en",
    );
    const labels = Array.from(h.el.querySelectorAll(".qd-grid dt")).map(
      (d) => d.textContent,
    );
    ...
    expect(
      h.el
        .querySelector(".qd-degradation")!
        .textContent!.replace(/\s+/g, " ")
        .trim(),
    ).toBe(
      "Degradation: the turn continued with required slots missing. Missing: thema.",
    );
  });
});
