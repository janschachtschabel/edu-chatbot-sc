...
async function mount(answers: Answers = {}): Promise<Harness> {
  ...
  http
    .expectOne((r) => r.url === HEALTH_URL)
    .flush(answers.health ?? { status: "ok" });
  http
    .expectOne((r) => r.url === FACTORY_URL)
    .flush(answers.factory ?? { exists: false });
  http.expectOne((r) => r.url === SNAPSHOTS_URL).flush(answers.snapshots ?? []);
  http
    .expectOne((r) => r.url === ELEMENTS_URL)
    .flush(answers.elements ?? ELEMENTS_NONE);
...
describe("OverviewComponent", () => {
  ...
  it("offers the overview and the architecture reference as two tabs", async () => {
    const h = await mount();
    expect(
      Array.from(h.el.querySelectorAll('[role="tab"]')).map((t) =>
        t.textContent?.trim(),
      ),
  ...
  it("mounts the reference on its first visit", async () => {
    ...
    await settle(h);
    h.http
      .expectOne((r) => r.url.includes("material-types"))
      .flush({ data: {} });
    await settle(h);
  ...
  it("says so when no eval has finished yet", async () => {
    const h = await mount({
      runs: [{ id: "r-1", status: "failed", avg_score: null }],
    });
  ...
  it("counts the elements on the layer cards once they arrive", async () => {
    const h = await mount({
      elements: {
        patterns: new Array(16).fill({}),
        personas: new Array(6).fill({}),
        intents: new Array(8).fill({}),
        states: new Array(3).fill({}),
        entities: new Array(5).fill({}),
        signals: new Array(17).fill({}),
      },
    });
