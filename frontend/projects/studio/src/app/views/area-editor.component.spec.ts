...
async function mount(
  ...
  TestBed.configureTestingModule({
    providers: [
      ...
      provideHttpClientTesting(),
      {
        ...
        useValue: {
          url: of(['bereich', ...AREA.split('/')].map((path) => new UrlSegment(path, {}))),
        },
      },
    ],
  });
  ...
  await tick();
  await fixture.whenStable();
  http.expectOne((r) => r.url === FILE_URL).flush({ path: `${AREA}.yaml`, content: raw });
  await tick();
  await fixture.whenStable();
...
function greetingInput(el: HTMLElement): HTMLInputElement {
  const control = el.querySelector<HTMLInputElement>('#ae-welcome\\.greeting, [id="ae-welcome.greeting"]');
...
describe('AreaEditorComponent — loading', () => {
  ...
  it('offers a retry instead of a dead page when loading fails', async () => {
    ...
    const http = TestBed.inject(HttpTestingController);
    await fixture.whenStable();
    http.expectOne(SCHEMA_URL).flush({ detail: 'unknown config area' }, { status: 404, statusText: 'x' });
    http.expectOne(DATA_URL).flush({ detail: 'x' }, { status: 404, statusText: 'x' });
    await tick();
    await fixture.whenStable();
    const el = fixture.nativeElement as HTMLElement;
    expect(el.textContent).toContain('Diesen Konfigurationsbereich gibt es nicht.');
...
describe('AreaEditorComponent — saving the form', () => {
  ...
  it('sends the whole document, not just the edited field', async () => {
    ...
    expect(req.request.body).toEqual({ data: { welcome: { greeting: 'Servus' } } });
    req.flush({ area: AREA, type: 'yaml', data: { welcome: { greeting: 'Servus' } } });
    await tick();
    await h.fixture.whenStable();
    // the YAML view would now be stale, so it is refetched
    h.http.expectOne((r) => r.url === FILE_URL).flush({
      path: `${AREA}.yaml`, content: 'welcome:\n  greeting: Servus\n',
    });
    await tick();
    await h.fixture.whenStable();
    expect(h.el.querySelector('.ae-status')?.textContent).toContain('Gespeichert.');
  });
  ...
  it('names the offending field when the backend rejects the document', async () => {
    ...
    button(h.el, 'Speichern').click();
    await h.fixture.whenStable();
    h.http.expectOne(DATA_URL).flush(
      { detail: [{ loc: ['body', 'welcome', 'quick_replies'], msg: 'Input should be a valid list' }] },
...
describe('AreaEditorComponent — the two tabs', () => {
  ...
  it('refuses to switch tabs while unsaved, and says why', async () => {
    ...
    expect(h.el.querySelector('#panel-ae-raw')?.hasAttribute('hidden')).toBe(true);
    expect(h.el.querySelector('.ae-status')?.textContent).toContain('speichern oder verwerfen');
  });
  ...
  it('saves the raw text through the file endpoint and reloads the document', async () => {
    ...
    button(h.el, 'Speichern').click();
    await h.fixture.whenStable();
    const put = h.http.expectOne((r) => r.url === FILE_URL && r.method === 'PUT');
    ...
    await tick();
    await h.fixture.whenStable();
    h.http.expectOne(DATA_URL).flush({ area: AREA, type: 'yaml',
  ...
  it('reports a YAML syntax error instead of failing silently', async () => {
    ...
    h.http
      ...
      .flush({ detail: 'Inhalt nicht lesbar: while parsing a flow node' },
             { status: 400, statusText: 'Bad Request' });
    await tick();
    await h.fixture.whenStable();
    expect(h.el.querySelector('[role="alert"]')?.textContent).toContain('Inhalt nicht lesbar');
  });
});
...
describe('AreaEditorComponent — switching to another area', () => {
  it('loads the second area — the component is reused, ngOnInit fires once', async () => {
    ...
    await fixture.whenStable();
    http.expectOne(SCHEMA_URL).flush(AREA_SCHEMAS[AREA]);
    http.expectOne(DATA_URL).flush({ area: AREA, type: 'yaml', data: { welcome: {} } });
    await tick();
    await fixture.whenStable();
    http.expectOne((r) => r.url === FILE_URL).flush({ path: `${AREA}.yaml`, content: 'welcome: {}' });
    await tick();
    await fixture.whenStable();

    url.next(['bereich', '01-base', 'policy'].map((p) => new UrlSegment(p, {})));
    await fixture.whenStable();
    http.expectOne('/studio/api/config/schema/01-base/policy').flush(AREA_SCHEMAS['01-base/policy']);
    http.expectOne('/studio/api/config/data/01-base/policy')
      .flush({ area: '01-base/policy', type: 'yaml', data: { rules: [] } });
    await tick();
    await fixture.whenStable();
    http.expectOne((r) => r.url === FILE_URL).flush({ path: '01-base/policy.yaml', content: '' });
    await tick();
    await fixture.whenStable();
    expect((fixture.nativeElement as HTMLElement).querySelector('.ae-title')?.textContent?.trim())
      .toBe('01-base/policy');
  });
  ...
  it('says so instead of loading forever when no area is named', async () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        provideHttpClient(),
        provideHttpClientTesting(),
        { provide: ActivatedRoute, useValue: { url: of([new UrlSegment('bereich', {})]) } },
      ],
    });
    const fixture = TestBed.createComponent(AreaEditorComponent);
    await fixture.whenStable();
    ...
    expect((fixture.nativeElement as HTMLElement).textContent)
      .toContain('Es ist kein Bereich angegeben');
  });
});
...
describe('AreaEditorComponent — a field that cannot be parsed blocks the save', () => {
  it('disables Speichern and names the field', async () => {
    ...
    await h.fixture.whenStable();
    expect(h.el.querySelector('[role="alert"]')?.textContent).toContain('welcome.irgendwas');
...
describe('AreaEditorComponent — markdown areas', () => {
  it('asks for the .md file when the document is a frontmatter/body pair', async () => {
    ...
    TestBed.configureTestingModule({
      providers: [
        ...
        provideHttpClientTesting(),
        {
          provide: ActivatedRoute,
          useValue: {
            url: of(['bereich', '01-base', 'base-persona'].map((p) => new UrlSegment(p, {}))),
          },
        },
      ],
    });
    ...
    const http = TestBed.inject(HttpTestingController);
    await fixture.whenStable();
    http.expectOne('/studio/api/config/schema/01-base/base-persona')
      .flush(AREA_SCHEMAS['01-base/base-persona']);
    http.expectOne('/studio/api/config/data/01-base/base-persona')
      .flush({ area: '01-base/base-persona', type: 'md',
    ...
    await tick();
    await fixture.whenStable();
    expect((fixture.nativeElement as HTMLElement).textContent).toContain('Markdown');
  });
});
