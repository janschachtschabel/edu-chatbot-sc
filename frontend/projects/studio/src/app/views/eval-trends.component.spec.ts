...
async function mount(
  payload: Record<string, unknown> | null = body(),
  locale = "de",
): Promise<void> {
...
describe("EvalTrendsComponent", () => {
  ...
  it("draws the score timeline plus one chart per classification series", async () => {
    ...
    for (const label of [
      "Cache-Hit-Rate",
      "LLM-Pattern-Übereinstimmung",
      "Persona-Trefferquote",
      "Intent-Trefferquote",
    ]) {
  ...
  it("skips an unjudged run in the chart but keeps its table row", async () => {
    await mount(
      body({ runs: [runMeta("r1", { avg_score: null }), runMeta("r2")] }),
    );
    ...
    expect(
      h.el.querySelectorAll("table.et-table tbody tr")[0].textContent,
    ).toContain("–");
  });
  ...
  it("zählt den Verlauf eines Patterns in der Mehrzahl der Sprache", async () => {
    ...
    expect(
      h.el.querySelector("svg.et-spark")!.getAttribute("aria-label"),
