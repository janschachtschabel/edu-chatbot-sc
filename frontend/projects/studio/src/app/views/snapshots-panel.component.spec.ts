...
const ROW = {
  id: 'snap-abc', created_at: '2026-07-20T09:30:00Z', label: 'vor dem Umbau',
  include_db: false,
};
...
describe('SnapshotsPanelComponent', () => {
  ...
  it('legt an, leert das Feld und liest die Liste neu', async () => {
    ...
    h.http.expectOne(LIST).flush([{ ...ROW, id: 'snap-neu', label: 'vor dem Remix' }]);
    await settle(h);
  ...
  it('fragt vor dem Wiederherstellen zurück und nennt den Snapshot', async () => {
    ...
    expect(h.el.querySelector('.op-confirm')!.getAttribute('role')).toBe('alert');
  ...
  it('zeigt den Satz des Backends, wenn das Anlegen scheitert', async () => {
    ...
    expect(h.el.querySelector('[role="alert"]')!.textContent)
      .toContain('Snapshot-Limit erreicht');
  });
});
