  ...
  return {
    ...
    conversations: [
      ...
      },
      {
        ...
        persona_id: "P-AND",
        intent_id: "I03",
        turns: [{ user: "Was ist OER?", bot: "OER heißt …" }],
      },
    ],
    ...over,
  };
...
async function mount(
  payload: Record<string, unknown> | null = detail(),
  id = "eval-abc",
  locale = "de",
): Promise<void> {
  ...
  if (payload === null)
    req.flush({ detail: "Lauf weg." }, { status: 404, statusText: "x" });
...
describe("EvalRunDetailComponent", () => {
  ...
  it("re-reads when the input names a different run", async () => {
    ...
    await h.fixture.whenStable();
    h.http
      .expectOne((r) => r.url === url("eval-xyz"))
      .flush(detail({ id: "eval-xyz" }));
    await settle();
  ...
  it("shows why a run failed instead of an empty scorecard", async () => {
    await mount(
      detail({
        status: "failed",
        error_message: "chat backend down",
  ...
  it("reports the judge average beside the hard rate, never inside it", async () => {
    await mount(
      detail({
        summary: {
          golden_metrics: goldMetrics({ judge_avg: 0.62, judged_turns: 3 }),
        },
      }),
    );
  ...
  it("calls a running run a snapshot and names what it is doing", async () => {
    await mount(
      detail({
        ...
        summary: {
          target_turns: 12,
          current_activity: "Gold-Flow GS-4 — Turn 2/3",
        },
      }),
    );
    ...
    expect(
      h.el.querySelector(".erd-live")?.textContent?.replace(/\s+/g, " ").trim(),
    ).toBe(
      "Momentaufnahme — der Lauf läuft noch. Gold-Flow GS-4 — Turn 2/3 " +
  ...
  it("re-reads on demand", async () => {
    ...
    await h.fixture.whenStable();
    h.http
      .expectOne((r) => r.url === url("eval-abc"))
      .flush(detail({ total_turns: 4 }));
    await settle();
  ...
  it("setzt die Auszeichnung im Satz als Element, nicht als sichtbaren Stern", async () => {
    await mount(
      detail({
        status: "running",
        completed_at: null,
        summary: { target_turns: 12 },
      }),
    );

    const strong = Array.from(h.el.querySelectorAll("strong")).map(
      (s) => s.textContent,
    );
  ...
  it("setzt den Variablennamen im Hinweis als <code>", async () => {
    await mount();
    const code = Array.from(h.el.querySelectorAll("code")).map(
      (c) => c.textContent,
    );
