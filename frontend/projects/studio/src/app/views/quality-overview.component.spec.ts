...
function flushDiagnosis(h: Harness): void {
  for (const path of ["degradations", "empty-entities", "low-confidence"]) {
    for (const req of h.http.match(
      (r) => r.url === `/studio/api/quality/${path}`,
    )) {
      req.flush({
        scope: "all",
        total: 0,
...
describe("QualityOverviewComponent", () => {
  ...
  it("does not show the two metrics that can never be non-zero", async () => {
    ...
    const labels = Array.from(h.el.querySelectorAll(".qo-kpi dt")).map(
      (dt) => dt.textContent!,
    );
  ...
  it("says why the score metrics are missing instead of silently dropping them", async () => {
    const h = await mount();
    expect(h.el.querySelector(".qo-note")!.textContent).toContain(
      "Score-Phase",
    );
  });
  ...
  it("warns only when a rate is actually above its threshold", async () => {
    ...
    expect(loud.el.querySelector(".qo-hints")!.textContent).toContain(
      `22,0${NBSP}%`,
    );
  });
  ...
  it("setzt den Prozentwert in den übersetzten Hinweissatz ein", async () => {
    ...
    expect(h.el.querySelector(".qo-hints li")!.textContent!.trim()).toBe(
      "Degradation rate at 22.0% — check the required slots of the " +
        "patterns concerned.",
    );
  });
