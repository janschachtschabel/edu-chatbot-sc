...
describe('ArchitectureReferenceComponent', () => {
  ...
  it('names each attribute group once instead of on every row', () => {
    ...
    expect(groupsOf('de')).toEqual(['Basis', 'Session', 'Kontext', 'Anzeige', 'Integration']);
    expect(groupsOf('en')).toEqual(['Basics', 'Session', 'Context', 'Display', 'Integration']);
  });
  ...
  it('übersetzt den Wissens-Abschnitt samt seinem Verweis', () => {
    ...
    expect(de.querySelector('a[href="/sicherung"]')?.textContent?.trim())
      .toBe('Bedient wird das alles in der Sicherung');

    const en = mount('en');
    expect(en.textContent).toContain('Knowledge sources: RAG and MCP');
    expect(en.textContent).toContain('Topic-page resolution');
    expect(en.querySelector('a[href="/sicherung"]')?.textContent?.trim())
      .toBe('All of this is operated in the backup view');
  });
  ...
  it('lässt Bezeichner in Ruhe, auch wenn die Zelle übersetzt wird', () => {
    ...
    for (const locale of ['de', 'en'] as const) {
      const codes = Array.from(mount(locale).querySelectorAll('code'))
        .map((c) => c.textContent);
