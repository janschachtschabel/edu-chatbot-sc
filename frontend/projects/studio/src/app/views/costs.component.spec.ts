...
const REPORT = {
  ...
  models: [
    {
      model: 'gpt-5.4-mini', calls: 5, prompt_tokens: 1_000_000,
      cached_tokens: 400_000, completion_tokens: 80_000, reasoning_tokens: 12_000,
      amount: '4.50',
    },
    {
      model: 'gpt-4o-mini', calls: 2, prompt_tokens: 234_567,
      cached_tokens: 0, completion_tokens: 9_000, reasoning_tokens: 0,
      amount: '0.42',
    },
  ],
  sessions: [
    {
      session_id: 'bb-teuer', calls: 4, prompt_tokens: 900_000,
      cached_tokens: 0, completion_tokens: 50_000, reasoning_tokens: 0,
      amount: '3.10', price_unavailable: [] as string[],
    },
  ],
};

const LEER = {
  empty: true, calls: 0, prompt_tokens: 0, cached_tokens: 0,
  completion_tokens: 0, reasoning_tokens: 0, currency: 'EUR',
  amount: null, price_unavailable: [] as string[], models: [], sessions: [],
};
...
async function mount(
  payload: Record<string, unknown> | null = REPORT, locale = 'de',
): Promise<void> {
  ...
  h = {
    fixture, el: fixture.nativeElement as HTMLElement, http,
...
async function answer(payload: Record<string, unknown> | null): Promise<void> {
  ...
  if (payload === null) req.flush({ detail: 'Datenbank weg' }, { status: 500, statusText: 'x' });
...
describe('Tagesgrenzen', () => {
  ...
  it('legt Anfang und Ende auf den Tag der Bedienung, nicht auf den UTC-Tag', () => {
    ...
    expect([start.getDate(), start.getHours(), start.getMinutes()]).toEqual([11, 0, 0]);
    expect([ende.getDate(), ende.getHours(), ende.getMinutes()]).toEqual([11, 23, 59]);
...
describe('Kostenschau — Zeitraum', () => {
  ...
  it('macht mit dem zurückliegenden Monat auf und fragt sofort', () => {
    ...
    expect([von.getHours(), von.getMinutes(), von.getSeconds()]).toEqual([0, 0, 0]);
    expect([bis.getHours(), bis.getMinutes(), bis.getSeconds()]).toEqual([23, 59, 59]);
  ...
  it('fragt beim Ändern einer Grenze neu', async () => {
    ...
    const von = new Date(h.query.get('from')!);
    expect([von.getFullYear(), von.getMonth(), von.getDate()]).toEqual([2026, 0, 1]);
...
describe('Kostenschau — Zahlen', () => {
  it('zeigt den Betrag in der Typografie der Oberfläche', async () => {
    await mount();
    expect(h.el.querySelector('.cst-amount-value')?.textContent?.trim())
      .toBe('4,92 €');
  });
  ...
  it('führt beide Tabellen mit ihren Zeilen', async () => {
    await mount();
    const kopfzeilen = Array.from(h.el.querySelectorAll('tbody th'),
...
describe('Kostenschau — Teilsumme', () => {
  ...
  it('zeigt ohne jeden Preis einen Strich und seinen Grund — keine Null', async () => {
    await mount({
      ...REPORT, amount: null, price_unavailable: ['gpt-5.4-mini'],
      models: [{ ...REPORT.models[0], amount: null }], sessions: [],
    });
  ...
  it('unterscheidet eine kaputte Preistafel von einer ungepflegten', async () => {
    ...
    // nur im Server-Log.
    await mount({
      ...REPORT, amount: null, price_config_broken: true,
  ...
  it('schreibt in eine Zeile ohne Preis den Grund statt einer leeren Zelle', async () => {
    await mount({
      ...REPORT, amount: '4.50', price_unavailable: ['gpt-4o-mini'],
      models: [REPORT.models[0], { ...REPORT.models[1], amount: null }], sessions: [],
    });
  ...
  it('sagt bei der Sitzungsliste, wonach sie ohne Preise ordnet', async () => {
    await mount({
      ...REPORT, amount: null, price_unavailable: ['gpt-5.4-mini'],
      sessions: [{ ...REPORT.sessions[0], amount: null, price_unavailable: ['gpt-5.4-mini'] }],
    });
...
describe('Kostenschau — Zustände', () => {
  ...
  it('spricht Englisch, wenn das Studio Englisch spricht', async () => {
    ...
    expect(h.el.querySelector('.cst-amount-value')?.textContent?.trim()).toBe('€4.92');
  });
});
