...
const ESTIMATE = {
  scenarios: 8, conversations: 4, total_turns: 20, chat_calls: 20,
  judge_calls: 20, simulator_calls: 16,
  est_usd: 0.14, est_usd_min: 0.08, est_usd_max: 0.28,
};
...
async function mount(config: Record<string, unknown> | null = CONFIG): Promise<void> {
  ...
  if (config === null) req.flush({ detail: 'weg' }, { status: 503, statusText: 'x' });
...
describe('EvalGenerativeStartComponent', () => {
  ...
  it('says why nothing can run when the config has no combinations', async () => {
    await mount({ personas: [{ id: 'P-LEH', label: 'Lehrkraft' }], intents: [] });
  ...
  it('starts the run with the checked values and reports it', async () => {
    ...
    await h.fixture.whenStable();
    const req = h.http.expectOne((r) => r.url === RUNS_URL && r.method === 'POST');
  ...
  it('surfaces warnings the backend attached to the start', async () => {
    ...
    button('Ja, Lauf starten')!.click();
    await h.fixture.whenStable();
    h.http.expectOne((r) => r.url === RUNS_URL && r.method === 'POST').flush({
      run_id: 'eval-abc', status: 'running',
