...
async function type(
  ...
  const field = h.el.querySelector<HTMLInputElement | HTMLTextAreaElement>(
    selector,
  );
...
async function pickMode(h: Harness, value: string): Promise<void> {
  const radio = h.el.querySelector<HTMLInputElement>(
    `input[name$="-mode"][value="${value}"]`,
  );
...
describe("RagIngestComponent", () => {
  ...
  it("names what is still missing instead of just greying the button out", async () => {
    const h = await mount();
    expect(h.el.querySelector(".ri-missing")?.textContent).toContain(
      "ein Wissensbereich und eine Datei",
    );
    ...
    expect(h.el.querySelector(".ri-missing")?.textContent).not.toContain(
      "Wissensbereich",
    );
  });
  ...
  it("uploads a chosen file into the named area", async () => {
    ...
    const body = h.http.expectOne("/studio/api/rag/ingest/file").request
      .body as FormData;
  ...
  it("sends a URL when the URL source is chosen", async () => {
    ...
    const body = h.http.expectOne("/studio/api/rag/ingest/url").request
      .body as FormData;
  ...
  it("keeps the entered text when the ingest fails", async () => {
    ...
    h.http
      .expectOne("/studio/api/rag/ingest/text")
      .flush(
        { detail: "Fehler beim Konvertieren" },
    ...
    expect(h.el.querySelector<HTMLTextAreaElement>(".ri-text")?.value).toBe(
      "Mühsam getippt",
    );
  });
  ...
  it("blocks a second send while the first is running", async () => {
    ...
    expect(h.el.querySelector<HTMLButtonElement>(".ri-go")?.disabled).toBe(
      true,
    );
    await submit(h);
