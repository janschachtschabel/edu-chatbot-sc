...
describe("RagApi", () => {
  ...
  it("caches the area list in a signal the whole page can read", async () => {
    ...
    http
      ...
      .flush([{ area: "wlo", chunks: 12, documents: 3 }]);
    await done;
  ...
  it("sends url and text ingests as form fields, not JSON", () => {
    ...
    expect(
      (
        http.expectOne("/studio/api/rag/ingest/url").request.body as FormData
      ).get("url"),
    ...
    expect(
      (
        http.expectOne("/studio/api/rag/ingest/text").request.body as FormData
      ).get("content"),
    ).toBe("Inhalt");
  });
  ...
  it("lässt den Satz des Backends stehen, wo es einen geschrieben hat", () => {
    ...
    const { t } = setup().lang;
    const abgelehnt = new StudioApiError(
      400,
      "Fehler beim Konvertieren: kaputt",
      "/x",
    );
    expect(describeRagError(abgelehnt, t)).toBe(
      "Fehler beim Konvertieren: kaputt",
    );
  });
});
