...
describe('StudioApi', () => {
  ...
  beforeEach(() => {
    TestBed.configureTestingModule({
      providers: [provideHttpClient(), provideHttpClientTesting(), StudioApi],
    });
  ...
  it('serializes query params and skips null/undefined', async () => {
    const done = api.get('/quality/logs', { limit: 50, scope: 'prod', persona: null });
  ...
  it('exposes the status CODE on failure, not just a message', async () => {
    ...
    http.expectOne('/studio/api/config/patterns')
  ...
  it('turns a network failure into status 0 with a readable German message', async () => {
    ...
    const err = (await done.then(() => null, (e: unknown) => e)) as StudioApiError;
  ...
  it('names the fields of a validation failure instead of "Unprocessable Content"', async () => {
    ...
    http.expectOne('/studio/api/config/data/01-base/welcome-config').flush(
      {
        detail: [
          { loc: ['body', 'welcome', 'quick_replies'], msg: 'Input should be a valid list' },
          { loc: ['body', 'welcome', 'tour_reply'], msg: 'Input should be a valid string' },
        ],
      },
      { status: 422, statusText: 'Unprocessable Content' },
    );
    const err = (await done.then(() => null, (e: unknown) => e)) as StudioApiError;
    expect(err.detail).toContain('welcome.quick_replies: Input should be a valid list');
  ...
  it('strips only the LEADING body segment — `body` is a real config key', async () => {
    ...
    http.expectOne('/studio/api/config/data/01-base/base-persona').flush(
      { detail: [{ loc: ['body', 'body'], msg: 'Input should be a valid string' }] },
      { status: 422, statusText: 'Unprocessable Content' },
    );
    const err = (await done.then(() => null, (e: unknown) => e)) as StudioApiError;
  ...
  it('sends put/post/delete to the same base', async () => {
    ...
    expect(http.expectOne('/studio/api/config/snapshots').request.method).toBe('POST');
    http.verify();
