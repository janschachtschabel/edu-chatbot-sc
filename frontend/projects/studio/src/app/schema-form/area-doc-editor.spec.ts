...
function answerLoad(area: string, data: Record<string, unknown>, type = 'yaml'): void {
...
describe('AreaDocEditor', () => {
  ...
  it('does not adopt a save answer that a newer request has overtaken', async () => {
    ...
    await tick();
    answerLoad(OTHER, { display_rules: { boxes: {} } });
    staleReq.flush({ area: AREA, data: { welcome: { greeting: 'eins' } }, type: 'yaml' });
  ...
  it('reports an unknown area in words an editor can act on', async () => {
    ...
    await tick();
    http.expectOne(schemaUrl('01-base/gibt-es-nicht'))
      .flush({ detail: 'unknown config area' }, { status: 404, statusText: 'Not Found' });
    http.expectOne(dataUrl('01-base/gibt-es-nicht'))
      .flush({ detail: 'unknown config area' }, { status: 404, statusText: 'Not Found' });
    await done;
  ...
  it('keeps the edit when the save is rejected', async () => {
    ...
    const saved = editor.save(AREA);
    await tick();
    http.expectOne(dataUrl(AREA)).flush(
      { detail: [{ loc: ['body', 'welcome', 'greeting'], msg: 'darf nicht leer sein' }] },
