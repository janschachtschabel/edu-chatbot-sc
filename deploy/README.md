...
| — | Master-Skill | Aus, solange `MASTER_SKILL_ENABLED` nicht `true` ist. Angeschaltet laedt der Agent-/Hybrid-Modus die Anleitung aus `MASTER_SKILL_NODE_ID` als Prompt-Kopf (Abnahme: eine Zeile `Master-Skill ... geladen` im Log des ersten Zuges). Scheitert der Abruf, antwortet der Chat ohne sie und warnt - kein Ausfall. |
